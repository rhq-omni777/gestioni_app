<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar proyecto</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Actualiza los datos del proyecto</p>
            </div>
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-100 dark:bg-gray-800 dark:ring-gray-700">
                <form method="POST" action="{{ route('projects.update', $project) }}" class="space-y-6 p-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $project->title) }}" required class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">{{ old('description', $project->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <select name="visibility" class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                                @foreach(['private' => 'Privado', 'team' => 'Equipo'] as $value => $label)
                                    <option value="{{ $value }}" @selected(old('visibility', $project->visibility) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('visibility')
                                <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" value="{{ old('due_date', optional($project->due_date)?->format('Y-m-d')) }}" class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white" />
                            @error('due_date')
                                <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-4 dark:border-gray-700">
                        ...
                        <button type="submit" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700">Actualizar proyecto</button>
                    </div>
                </form>
            </div>
        </div>
