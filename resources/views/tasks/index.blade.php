<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Tareas</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Seguimiento de pendientes y responsables</p>
            </div>
            <a href="{{ route('tasks.create') }}" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700">Nueva tarea</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-100 dark:bg-gray-800 dark:ring-gray-700">
                <div class="border-b border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-900">
                    <form method="GET" action="{{ route('tasks.index') }}" class="grid gap-3 md:grid-cols-6">
                        ...
                        <select name="project_id" class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                            <option value="">Proyecto</option>
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" @selected(request('project_id')==$project->id)>{{ $project->title }}</option>
                            @endforeach
                        </select>
                        <select name="assigned_to" class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        ...
                        <select name="status" class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                            ...
                            @foreach(['todo'=>'Por hacer','doing'=>'En progreso','done'=>'Completada'] as $value=>$label)
                        ...
                        <div class="flex flex-wrap gap-2 md:col-span-6 justify-end">
                            <a href="{{ route('tasks.index') }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-900">Limpiar</a>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">Proyecto</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                            @forelse($tasks as $task)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                                    <td class="px-4 py-3 text-sm font-semibold text-gray-900 dark:text-white">{{ $task->title }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ $task->project?->title ?? 'Sin proyecto' }}</td>
                                    <td class="px-4 py-3 text-sm">
                                        <span class="inline-flex rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700 dark:bg-indigo-900 dark:text-indigo-100">{{ ucfirst($task->status) }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ ucfirst($task->priority) }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ $task->assignee?->name ?? 'Sin asignar' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ optional($task->due_date)->format('d/m/Y') ?? 'Sin fecha' }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex justify-end gap-3 text-sm">
                                            <a href="{{ route('tasks.show', $task) }}" class="font-semibold text-sky-600 hover:text-sky-800 dark:text-sky-200 dark:hover:text-sky-100">Ver</a>
                                            <a href="{{ route('tasks.edit', $task) }}" class="font-semibold text-indigo-600 hover:text-indigo-800 dark:text-indigo-200 dark:hover:text-indigo-100">Editar</a>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-300">Aún no hay tareas</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
